  ...
  if (save_plots && nrow(results_df) > 0) {
    tryCatch({
      plot_cutoff_composite_summary_v2(
        results_df,
        out_pdf = file.path(plot_dir, "test_composite_score_plot.pdf"),
        top_n = NULL,
        order_by = "composite_score"
      )
    }, error = function(e) {
      warning("Composite summary plot failed, but results_df is retained: ",
              conditionMessage(e))
    })
    ...
    if (nrow(top_genes) > 0) {
      ...
      tryCatch({
        plot_cutoff_composite_summary_v2(
          results_df = top_genes,
          out_pdf = file.path(plot_dir, "top_only", "test_top_genes_composite_score_plot.pdf"),
          top_n = NULL,
          order_by = "composite_score"
        )
      }, error = function(e) {
        warning("Top-gene composite summary plot failed, but results_df is retained: ",
                conditionMessage(e))
      })
    }
  }
